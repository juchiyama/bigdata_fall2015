...
class ImportNEsTestCase(unittest.TestCase):

	def setUp(self):
		self.t = self.assertTrue
		self.inst = self.assertIsInstance
		self.inserter = BulkInserter(host="localhost",port=27017,database="reddit_stream_test",collection="ners")
	def tearDown(self):	
		self.inserter.source.drop()

	def test_keyfix_characters(self):
		count = 0
		for ind, user in enumerate(annotation.ner_reader("data/test_ner.json")):
